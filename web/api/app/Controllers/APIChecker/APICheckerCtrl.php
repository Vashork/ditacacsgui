...
################################################
	...
	public function getCheckDatabase(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
	{
		//INITIAL CODE////START//
		$data=array();
		...
		/////////////////////////
		$updateFlag = (@$this->param($request, 'update')) ? 1 : 0;
		//var_dump( $this->db::getSchemaBuilder()->getColumnType('tac_users', 'username') ); die;
		//var_dump($request->getParam('update'));die();
		$data['messages'] = array( );
			...
			foreach($tablesArr as $tableName => $tableColumns){
				...
				if(!$this->db::connection($database)->getSchemaBuilder()->hasTable($tableName))
				{
					...
					if ($updateFlag) {
						...
						///////////////////////////////////
						//$this->databaseFix();
						sleep(1); return $this->json($response, $data, 200);
					}
				}
 	...
 	################################################
 	###################API STATUS MAIN##############
 	public function getApiStatus(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 	...
 	################################################
 	###################API TIME MAIN##############
 	public function getApiTime(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
