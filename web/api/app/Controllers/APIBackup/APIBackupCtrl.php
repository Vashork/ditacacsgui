...
class APIBackupCtrl extends Controller
{
 ...
 private $listOfTacacsReportsTables = '--tables tac_log_accounting tac_log_authorization tac_log_authentication';
 	...
 	public function postBackupDatatables(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 	...
 	public function postBackupDelete(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 		...
 		// $data['result'] = trim(shell_exec(TAC_ROOT_PATH . '/backup.sh delete '. $request->getParam('name')));
 		$data['result'] = CMDRun::init()->setCmd(TAC_ROOT_PATH . '/backup.sh')->
 				setAttr(['delete',$this->param($request, 'name')])->get();

 		$logEntry=array('action' => 'delete', 'obj_name' => $this->param($request, 'name'), 'section' => 'api backup', 'message' => 602);
 	...
 	public function getBackupSettings(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 	...
 	public function postBackupSettings(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 	...
 	public function postBackupRestore(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 		...
 		$data['cmd'] = CMDRun::init()->setCmd(TAC_ROOT_PATH . '/backup.sh')->setAttr(['restore',DB_USER,DB_PASSWORD, DB_NAME, $this->param($request, 'name')])->showCmd();
 		$data['result'] = CMDRun::init()->setCmd(TAC_ROOT_PATH . '/backup.sh')->setAttr(['restore',DB_USER,DB_PASSWORD, DB_NAME, $this->param($request, 'name')])->get();

 		if ( !empty($type) AND $type != 'apicfg' ) $data['changeConfiguration']=$this->changeConfigurationFlag(['unset' => 0]);

 		$logEntry=array('action' => 'restore', 'obj_name' => $this->param($request, 'name'), 'section' => 'api backup', 'message' => 603);
 		$data['logging']=$this->APILoggingCtrl->makeLogEntry($logEntry);
 	...
 	public function getBackupDownload(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 	...
 	public function postBackupUpload(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
 		...
 		$action = $this->param($request, 'action');
 		if ($action == 'check') {
 			if ( file_exists($uploaddir. $this->param($request, 'name')) ) {
 	...
 	public function postBackupMake(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
 	{
 		//INITIAL CODE////START//
 		$data=array();
 		$data=$this->initialData([
